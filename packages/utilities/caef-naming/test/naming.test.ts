...
import { CaefDefaultResourceNaming, CaefResourceNamingConfig } from "../lib";
import { App } from "aws-cdk-lib";

describe( "CaefDefaultResourceNaming", () => {

    const namingProps: CaefResourceNamingConfig = {
    ...
    const naming = new CaefDefaultResourceNaming( namingProps )
    ...
    test( "resourceName", () => {
        ...
        expect( naming.resourceName( "${Token[TOKEN.123]}" ) ).toBe( "test-org-test-env-test-domain-test-module-${Token[TOKEN.123]}" )
    ...
    test( "ssmPath", () => {
        ...
        expect( naming.ssmPath( "${Token[TOKEN.123]}", true, true ) ).toBe( "/test-org/test-domain/test-module/${Token[TOKEN.123]}" )
    } )
    ...
    test( "exportName", () => {
        ...
        expect( naming.exportName( "${Token[TOKEN.123]}", true, true ) ).toBe( "test-org:test-domain:test-module:${Token[TOKEN.123]}" )
    } )
    ...
    test( "withModuleName", () => {
        expect( naming.withModuleName( "new-test-module" ).resourceName() ).toBe( "test-org-test-env-test-domain-new-test-module" )
    } )
