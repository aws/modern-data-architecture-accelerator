...
describe( 'Test BucketPolicy Helper', () => {
    ...
    describe( 'RestrictPrefix', () => {
        ...
        test( "Read Role Ids", () => {
            ...
            expect( restriction.readStatements()[ 0 ].resources ).toStrictEqual( [ "arn:test-partition:s3:::test-bucket/test-prefix/*" ] )
        } )
        ...
        test( "Read Principals", () => {
            const testProps: IRestrictObjectPrefixToRoles = {
                ...baseTestProps,
                readPrincipals: [ 
                    new ArnPrincipal("test-role-arn-1")
                ]
            }
            const restriction = new RestrictObjectPrefixToRoles( testProps )
            // console.log( JSON.stringify( restriction.statements()[ 0 ], undefined, 2 ) )
        ...
        test( "ReadWrite Principals", () => {
            ...
            expect( restriction.readWriteStatements()[ 0 ].resources ).toStrictEqual( [ "arn:test-partition:s3:::test-bucket/test-prefix/*" ] )
        ...
        test( "ReadWriteSuper Principals", () => {
            ...
            expect( restriction.readWriteSuperStatements()[ 0 ].resources ).toStrictEqual( [ "arn:test-partition:s3:::test-bucket/test-prefix/*" ] )
    ...
    describe( 'RestrictBucket', () => {
        ...
        test( "Base Allow", () => {
            ...
            expect( restriction.allowStatement.actions ).toStrictEqual( [
                "s3:List*",
                "s3:GetBucket*"
            ] )
        ...
        test( "Base Deny", () => {
            ...
            expect( restriction.denyStatement.actions ).toStrictEqual( [
                "s3:PutObject*",
                "s3:GetObject*",
                "s3:DeleteObject*"
            ] )
