...
import { CaefDefaultResourceNaming } from "@aws-caef/naming"
...
export class CaefTestApp extends App {

    public readonly naming: CaefDefaultResourceNaming
    ...
    constructor( testContext: { [ key: string ]: string } = {} ) {
        super( {
            context: CaefTestApp.createContext( testContext )
        } )
        ...
        const namingProps = {
            ...{
                cdkNode: this.node
            }, ...CaefTestApp.createContext( testContext )
        }
        this.naming = new CaefDefaultResourceNaming( namingProps )
    ...
    public checkCdkNagCompliance ( stack: Stack ) {
        describe( 'CAEF CDK Nag Compliance Tests', () => {
