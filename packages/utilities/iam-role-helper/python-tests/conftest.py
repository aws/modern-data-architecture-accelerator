"""
Shared pytest fixtures for IAM Role Helper Lambda tests.
"""
import pytest
import os
import sys
...
resolve_role_src_path = os.path.join(os.path.dirname(__file__), '..', 'src', 'python', 'resolve_role')
sys.path.insert(0, resolve_role_src_path)
...
def lambda_context():
    ...
    class MockContext:
        def __init__(self):
            self.function_name = "resolve-role-function"
...
def create_event():
    ...
    return {
        ...
        "ResourceType": "Custom::ResolveRole",
        "ResourceProperties": {
            "roleRef": {
                "name": "test-role"
            }
