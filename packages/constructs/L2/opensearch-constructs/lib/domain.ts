        ...
        this.node.children.forEach( child => {
            ...
            if ( child.node.id.startsWith( "Resource" ) ) {
                NagSuppressions.addResourceSuppressions(
                    child,
                    [
                        { id: 'AwsSolutions-OS3', reason: 'The Opensearch domain is deployed within VPC. IP based access policies cannot be applied to domains that reside within VPC because security groups already enforce IP-based access policy. https://docs.aws.amazon.com/opensearch-service/latest/developerguide/vpc.html#vpc-security' },
                        { id: 'AwsSolutions-OS5', reason: 'CDK Construct applies Domain Access Policy immedietely after the domain is created. Zero Trust is still applied, not as part of domain properties in CDK generated cloudformation template, but immediately after domain deployment via custom resource' }
                    ]
                );
            }
                ...
                child.node.children.forEach( child2 => {
                    if ( child2.node.id.includes( 'ServiceRole' ) ) {
                        NagSuppressions.addResourceSuppressions(
                            child2,
                            [
                                { id: 'AwsSolutions-IAM4', reason: 'Role is for Custom Resource Provider Lambda Function. AWS Managed policy AWSLambdaBasicExecutionRole provides least privilege permissions.' }
                            ]
                        );
                    }
