...
export class MdaaRdsServerlessCluster extends DatabaseCluster {
    ...
    constructor( scope: Construct, id: string, props: MdaaRdsServerlessClusterProps ) {
        ...
        const rotationScheduleInDays =  (props.adminPasswordRotationDays!=undefined) && (props.adminPasswordRotationDays>0) ?  props.adminPasswordRotationDays : 30
    ...
    private addOverrides ( cfnDbCluster: CfnDBCluster, suppressions: NagPackSuppression[] ) {
        const _enableCloudwatchLogsExports: boolean = this.props.enableCloudwatchLogsExports != undefined ? this.props.enableCloudwatchLogsExports : true
        
        // EnableCloudwatchLogsExports Override
        if ( _enableCloudwatchLogsExports ) {
            const dbEngineCwLogsExports: string[] = this.props.engine == 'aurora-mysql' ? [ 'audit', 'error', 'general', 'slowquery' ] : [ 'postgresql' ]
        ...
        if ( this.props.engine == 'aurora-mysql' ) {
            const _backtrackWindowInSeconds: number = this.props.backtrackWindowInSeconds != undefined ? this.props.backtrackWindowInSeconds : 86400
            ...
            if ( _backtrackWindowInSeconds ) {
                ...
                suppressions.push( { id: 'AwsSolutions-RDS14', reason: 'Remediated through property override. Currently, Backtrack is only supported for Aurora MySQL DB clusters' } )
            }
        }
        // Port Override
