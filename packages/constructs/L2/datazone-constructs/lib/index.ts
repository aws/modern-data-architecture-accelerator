...
export class MdaaDatazoneProject extends Construct {
    ...
    constructor( scope: Construct, id: string, props: MdaaDatazoneProjectProps ) {
        ...
        const domainConfigParser = props.domainConfigSSMParam ? MdaaDatazoneProject.createSsmDomainConfigParser( scope, props.naming, props.domainConfigSSMParam ) : undefined
        ...
        const domainCustomEnvBlueprintId = props.domainCustomEnvBlueprintId || domainConfigParser?.getAttString('datalakeEnvBlueprintId')
        if(!domainCustomEnvBlueprintId) throw new Error("domainCustomEnvBlueprintId must either be defined directly in props, or via domainConfigSSMParam")
        this.domainCustomEnvBlueprintId = domainCustomEnvBlueprintId

        const datalakeManagementRoleArn = props.datalakeManagementRoleArn || domainConfigParser?.getAttString('datalakeManagementRoleArn')
        if(!datalakeManagementRoleArn) throw new Error("datalakeManagementRoleArn must either be defined directly in props, or via domainConfigSSMParam")
        ...
        if(!adminUserProfileId) throw new Error("adminUserProfileId must either be defined directly in props, or via domainConfigSSMParam")
    ...
    private static createSsmDomainConfigParser ( scope: Construct, naming: IMdaaResourceNaming, domainConfigSSMParam:string ): MdaaCustomResource {
        const ssmParamArn = domainConfigSSMParam.startsWith( "arn:" ) ? domainConfigSSMParam :
            `arn:${ Stack.of( scope ).partition }:ssm:${ Stack.of( scope ).region }:${ Stack.of( scope ).account }:parameter${ domainConfigSSMParam }`
