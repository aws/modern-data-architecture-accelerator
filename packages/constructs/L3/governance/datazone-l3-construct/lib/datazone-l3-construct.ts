...
export class DataZoneL3Construct extends MdaaL3Construct {
    ...
    private static CUSTOM_ENV_BLUEPRINT_ID: string = "dqsdikgj5tspu2";
    ...
    private createDomain(domainName: string, domainProps: DomainProps) {
        ...
        const customEnvBlueprintConfig = this.createCustomBlueprintConfig(this,domain.attrId,[this.region],kmsKey.keyArn)
        ...
        const configParam = this.createDomainConfigParam(domainName, domain, adminUserProfile, customEnvBlueprintConfig.getAttString("id"))
        ...
        if (domainProps.associatedAccounts) {
            ...
            if (configParam.param) {
                const ramShareProps: CfnResourceShareProps = {
                    ...
                    resourceArns: [
                        configParam.param.parameterArn
                    ],
                    principals: Object.entries(domainProps.associatedAccounts).map(x => x[1].account),
                }
            ...
            if (domainArnParam.param) {
                const ramShareProps: CfnResourceShareProps = {
                    ...
                    resourceArns: [
                        domainArnParam.param.parameterArn
                    ],
                    principals: Object.entries(domainProps.associatedAccounts).map(x => x[1].account),
                }
            ...
            if (domainIdParam.param && kmsKeyArnParam.param) {
                const domainIdShareProps: CfnResourceShareProps = {
                    ...
                    resourceArns: [
                        domainIdParam.param.parameterArn
                    ],
                    principals: Object.entries(domainProps.associatedAccounts).map(x => x[1].account),
                }
                ...
                const domainKeyArnShareProps: CfnResourceShareProps = {
                    ...
                    resourceArns: [
                        kmsKeyArnParam.param.parameterArn
                    ],
                    principals: Object.entries(domainProps.associatedAccounts).map(x => x[1].account),
                }
                ...
                Object.entries(domainProps.associatedAccounts).forEach(associatedAccount => {
                    const accountCdkUserProfileProps: CfnUserProfileProps = {
                        domainIdentifier: domain.attrId,
                        userIdentifier: associatedAccount[1].cdkRoleArn ?? `arn:${this.partition}:iam::${associatedAccount[1].account}:role/cdk-hnb659fds-cfn-exec-role-${associatedAccount[1].account}-${this.region}`,
                        userType: 'IAM_ROLE',
                        status: 'ACTIVATED'
                    }
                    new CfnUserProfile(this, `${domainName}-${associatedAccount[1].account}-cdk-user-profile`, accountCdkUserProfileProps)
                    ...
                    if(this.props.crossAccountStacks) {
                        ...
                        if(crossAccountStack) {
                            const domainIdSsmParamArn = `arn:${this.partition}:ssm:${associatedAccount[1].region ?? this.region}:${this.account}:parameter${domainIdParam.paramName}`
                            const kmsKeyArnParamArn = `arn:${this.partition}:ssm:${associatedAccount[1].region ?? this.region}:${this.account}:parameter${kmsKeyArnParam.paramName}`
                            this.createCustomBlueprintConfig(
                                crossAccountStack,
                                StringParameter.fromStringParameterArn(crossAccountStack,`domain-id-import-${associatedAccount}`,domainIdSsmParamArn).stringValue,
                                [associatedAccount[1].region || this.region],
                                StringParameter.fromStringParameterArn(crossAccountStack,`domain-key-arn-id-import-${associatedAccount}`,kmsKeyArnParamArn).stringValue,
                            )
                        } else {
                            console.warn(`Cross account stack not defined for associated account ${associatedAccount[0]}/${associatedAccount[1].account} on domain ${domainName}`)
                        }
                    }
                })
            }
    ...
    private createDomainConfigParam(
        ...
        customEnvBlueprintConfigId: string): MdaaParamAndOutput {
    ...
    private createDomainKmsKey(domainName: string, domainProps: DomainProps, dataAdminRole: MdaaResolvableRole): IKey {
        ...
        const keyAccessAccounts = [...Object.entries(domainProps.associatedAccounts || {}).map(x => x[1].account), this.account]
    ...
    private createCustomBlueprintConfig(scope:Construct,domainId:string,regions:string[],domainKmsKeyArn:string): MdaaCustomResource {
        ...
        const crProps: MdaaCustomResourceProps = {
            ...
            handlerPolicySuppressions: [{
                ...
                reason: "PutEnvironmentBlueprintConfiguration does not take a resource: https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazondatazone.html "
            }],
            handlerProps: {
