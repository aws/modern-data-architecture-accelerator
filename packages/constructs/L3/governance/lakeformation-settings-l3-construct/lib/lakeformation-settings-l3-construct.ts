...
export class LakeFormationSettingsL3Construct extends MdaaL3Construct {
    ...
    private createIdcConfig(boto3Layer: MdaaBoto3LayerVersion){
        ...
        const manageRAMPolicyStatement = new PolicyStatement( {
            effect: Effect.ALLOW,
            resources: [ 
                `arn:${this.partition}:ram:${this.region}:${this.account}:resource-share/*`
            ],
            actions: [
        ...
        const idConfigCrProps: MdaaCustomResourceProps = {
            ...
            handlerRolePolicyStatements: [ manageIdcConfigsPolicyStatement,manageSsoAppPolicyStatement,manageRAMPolicyStatement ],
            handlerPolicySuppressions: [
                ...
                { id: 'AwsSolutions-IAM5', reason: 'SSO App name not known at deployment time. https://docs.aws.amazon.com/service-authorization/latest/reference/list_awslakeformation.html#awslakeformation-actions-as-permissions' }
            ],
    ...
    private createLFSettings(boto3Layer: MdaaBoto3LayerVersion){
        ...
        const cdkLfAdmin = {
            ...
            DataLakePrincipalIdentifier: synthesizer.cloudFormationExecutionRoleArn.replace('${AWS::Partition}', this.partition),
        }
