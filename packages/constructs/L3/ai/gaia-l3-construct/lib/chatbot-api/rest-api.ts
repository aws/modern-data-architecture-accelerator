...
export class RestApi extends MdaaL3Construct {
  ...
  private addApiHandlerRoleAuroraRagPermissions(apiHandlerRole: MdaaRole, apiHandler: MdaaLambdaFunction) {
    if (this.props.ragEngines?.auroraPgVector) {
      ...
      this.props.ragEngines.auroraPgVector.database.connections.allowDefaultPortFrom(apiHandler);

      this.props.ragEngines.auroraPgVector.createAuroraWorkspaceWorkflow.grantStartExecution(apiHandlerRole);
    }
  }
