...
def set_workspace_prompt_template(
        workspace_id: str,
    ...
    if not instructions:
        raise genai_core.types.CommonError("Template instructions is required")
    prompt_template_data = {"instructions": instructions}
    if bot_name:
        prompt_template_data.update({"bot_name": bot_name})
    if examples:
        prompt_template_data.update({"examples": [dict(example_pair) for example_pair in examples]})
    timestamp = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ")

    table.update_item(
        Key={"workspace_id": workspace_id, "object_type": WORKSPACE_OBJECT_TYPE},
        UpdateExpression="SET prompt_template=:promptTemplate, updated_at=:timestampValue",
        ExpressionAttributeValues={
            ":timestampValue": timestamp,
            ":promptTemplate": json.dumps(prompt_template_data),
        }
    )

    return get_workspace_prompt_template(workspace_id)


def get_workspace_prompt_template(workspace_id: str):
    response = table.get_item(
        Key={"workspace_id": workspace_id, "object_type": 'workspace'}
    )
    prompt_blob = response.get("Item").get("prompt_template")

    if not prompt_blob:
        return None

    item = json.loads(prompt_blob)

    return {"botName": item.get("bot_name"),
...
def delete_workspace_prompt_template(workspace_id: str):
    table.update_item(
        Key={"workspace_id": workspace_id, "object_type": WORKSPACE_OBJECT_TYPE},
        UpdateExpression="REMOVE prompt_template",
    )
...
def create_workspace_bedrock_kb(
    workspace_name: str, knowledge_base: dict
):
    workspace_id = str(uuid.uuid4())
    timestamp = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ")
