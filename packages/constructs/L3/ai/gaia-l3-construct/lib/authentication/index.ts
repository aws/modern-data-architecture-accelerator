...
export class Authentication extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AuthenticationProps) {
    ...
    if (props.config.auth?.existingPoolId !== undefined) {
      userPool = cognito.UserPool.fromUserPoolId(this, 'ExistingUserPool', props.config.auth.existingPoolId);
    } else {
    ...
    if (props.config.auth?.existingPoolClientId !== undefined) {
      ...
      if (
        props.config.auth.authType === SupportedAuthTypes.ACTIVE_DIRECTORY &&
        ...
        userPoolClient = userPool.addClient('UserPoolClient', {
          ...
          supportedIdentityProviders: [cognito.UserPoolClientIdentityProvider.custom(adProvider.providerName)],
        });
      } else {
        userPoolClient = userPool.addClient('UserPoolClient', {
          generateSecret: false,
    ...
    // ISSUE-507 UserpoolDomain is not always defined. Not sure what the domain is
    let userPoolDomain = {
      domainName: 'unknown',
    } as cognito.IUserPoolDomain;
    ...
    if (props.config.auth?.existingPoolDomain !== undefined) {
      ...
      if (props.config.auth?.cognitoDomain) {
