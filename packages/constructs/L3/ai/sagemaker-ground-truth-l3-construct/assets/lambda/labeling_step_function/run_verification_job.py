...
def handler(event: Dict[str, Any], context: object) -> Dict[str, str]:
    ...
    # Determine UI config: custom template or built-in HumanTaskUi
    ui_kwargs = {}
    if INSTRUCTIONS_TEMPLATE_S3_URI:
        ui_kwargs["instructions_template_s3_uri"] = INSTRUCTIONS_TEMPLATE_S3_URI
    elif HUMAN_TASK_UI_NAME:
        # 394669845002 is AWS's SageMaker-managed account hosting built-in Human Task UI ARNs
        # See: https://docs.aws.amazon.com/sagemaker/latest/dg/sms-ui-template-reference.html
        ui_kwargs["human_task_ui_arn"] = f"arn:aws:sagemaker:{AWS_REGION}:394669845002:human-task-ui/{HUMAN_TASK_UI_NAME}"
    else:
