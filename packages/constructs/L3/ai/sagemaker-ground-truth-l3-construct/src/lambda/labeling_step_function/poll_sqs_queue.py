...
def handler(event: Dict[str, Any], context: object) -> Dict[str, str]:
    ...
    return {
        "MessagesCount": len(record_source_to_receipt_handle),
...
def poll_sqs_queue(
    ...
    malformed_receipt_handles: List[str] = []
        ...
        for message in response["Messages"]:
            try:
                if task_media_type == IMAGE:
                    ...
                    # EventBridge S3 Object Created event format
                    detail = body.get("detail", {})
                    bucket_name = detail.get("bucket", {}).get("name")
                    object_key = detail.get("object", {}).get("key")
                    if not bucket_name or not object_key:
                        logger.error(f"Missing bucket/key in EventBridge event: {body}")
                        malformed_receipt_handles.append(message["ReceiptHandle"])
                        continue
    ...
    if malformed_receipt_handles:
        logger.warning(f"Deleting {len(malformed_receipt_handles)} malformed messages from the queue.")
        delete_duplicate_record_source_messages(
            sqs_queue_url=sqs_queue_url,
            messages_per_batch=messages_per_batch,
            receipt_handles_of_duplicates=malformed_receipt_handles,
        )
...
def delete_duplicate_record_source_messages(
    ...
    for batch in receipt_handles_batches:
        try:
            ...
            if failed:
                ...
                errors.append(f"Partial batch failure: {failed_ids}")
                logger.error(f"Failed to delete {len(failed)} messages: {failed_ids}")
    ...
    if errors:
        raise RuntimeError(
            f"Failed to delete some messages from queue: {'; '.join(errors)}"
        )
