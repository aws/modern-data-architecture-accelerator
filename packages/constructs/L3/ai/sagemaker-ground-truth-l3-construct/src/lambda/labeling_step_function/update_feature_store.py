...
boto3_config = Config(connect_timeout=5, read_timeout=30, retries={"max_attempts": 2}, max_pool_connections=THREAD_POOL_SIZE)
...
def batch_delete_message_from_sqs_and_save_to_feature_store(
    ...
    if failed_deletes:
        failed_ids = [f["Id"] for f in failed_deletes]
        logger.error(f"Failed to delete {len(failed_deletes)} messages from SQS: {failed_ids}")
    ...
    if "Successful" in response:
        ...
        # Wait for all feature store writes and re-raise any failures
        for future in concurrent.futures.as_completed(futures):
            future.result()  # raises if put_record_to_feature_store failed

    if failed_deletes:
        raise RuntimeError(
            f"Failed to delete {len(failed_deletes)} SQS messages. "
            f"Feature store writes for successfully deleted messages completed. "
            f"Failed message IDs: {[f['Id'] for f in failed_deletes]}"
        )


def put_record_to_feature_store(feature_group_name: str, record: List[Dict[str, str]]) -> None:
    """Write a single record to the Feature Store. Raises on failure to prevent silent data loss."""
