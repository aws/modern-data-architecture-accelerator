...
import { BedrockAgentL3Construct, BedrockAgentL3ConstructProps, BedrockAgentProps, LambdaFunctionProps } from '../lib';
...
describe('Mdaa Compliance Stack Tests', () => {
  ...
  describe('Bedrock Agent L3 Construct Compliance Tests', () => {
    ...
    test('Test Bedrock Agent Alias', () => {
      template.hasResourceProperties('AWS::Bedrock::AgentAlias', {
        AgentAliasName: 'test-alias',
        AgentId: {
          'Fn::GetAtt': ['teststackmdaabedrockagenttestagent1260C3FCD', 'AgentId'],
        },
      });
    });
    ...
    test('Test Lambda Invoke Permission for Bedrock Agent', () => {
      template.hasResourceProperties('AWS::Lambda::Permission', {
        ...
        SourceArn: {
          'Fn::GetAtt': ['teststackmdaabedrockagenttestagent1260C3FCD', 'AgentArn'],
        },
      });
    });
  });
  ...
  describe('Bedrock Agent Compliance Test with existing Lambda Functions', () => {
    const template = generateTemplateFromTestInput(dataAdminRoleRef, agentExecutionRoleRef, {
      'test-agent-2': agent_2,
    });
    // console.log(JSON.stringify(template, undefined, 2));
    test('Test Bedrock Agent Resource', () => {
      template.hasResourceProperties('AWS::Bedrock::Agent', {
        ...
        CustomerEncryptionKeyArn: {
          'Fn::GetAtt': ['bedrockagentcmkC1E0055A', 'Arn'],
        },
        Description: 'Sample Agent  2',
        FoundationModel: 'anthropic.claude-3-sonnet-20240229-v1:0',
        GuardrailConfiguration: {
...
function generateTemplateFromTestInput(
  ...
  execRoleRef: MdaaRoleRef,
  agents: { [agentName: string]: BedrockAgentProps },
  ...
  new BedrockAgentL3Construct(testApp.testStack, 'teststack', constructProps);
