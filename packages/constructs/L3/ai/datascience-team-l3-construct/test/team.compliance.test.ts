...
import { InventoryDefinition } from '@aws-mdaa/datalake-l3-construct';
import { CENTRAL_SM_REPO_ACCT, DataScienceTeamL3Construct, DataScienceTeamL3ConstructProps } from '../lib';
    ...
    test('Test SageMakerDescribeGet', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:Describe*', 'sagemaker:BatchDescribe*', 'sagemaker:Get*', 'sagemaker:BatchGet*'],
              Effect: 'Allow',
              Resource: '*',
        ...
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              ...
              Effect: 'Allow',
              Resource: [
                'arn:test-partition:sagemaker:test-region:test-account:*job/*',
                'arn:test-partition:sagemaker:test-region:test-account:*job-definition/*',
              ],
    ...
    test('Test CreateandManageModelCards', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:CreateModelCard', 'sagemaker:DeleteModelCard', 'sagemaker:UpdateModelCard'],
              Effect: 'Allow',
    ...
    test('Test CreateAndManageModels', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:CreateModel', 'sagemaker:DeleteModel'],
              Effect: 'Allow',
        ...
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              ...
              Effect: 'Allow',
              Resource: [
                'arn:test-partition:sagemaker:test-region:test-account:endpoint/*',
                'arn:test-partition:sagemaker:test-region:test-account:endpoint-config/*',
              ],
              Action: [
    ...
    test('Test CreateAndManageProjects', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:CreateProject', 'sagemaker:DeleteProject', 'sagemaker:UpdateProject'],
              Effect: 'Allow',
    ...
    test('Test PutRecordFeatureGroups', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:PutRecord', 'sagemaker:DeleteRecord'],
              Effect: 'Allow',
    ...
    test('Test AddDeleteTagsSageMaker', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:AddTags', 'sagemaker:DeleteTags'],
              Effect: 'Allow',
              ...
              Condition: {
                StringEquals: {
                  'sagemaker:FeatureGroupOnlineStoreKmsKey': {
                    'Fn::GetAtt': ['teststackcmk3D411286', 'Arn'],
                  },
                },
              },
              Effect: 'Allow',
                ...
                StringLike: {
                  'sagemaker:FeatureGroupOfflineStoreS3Uri': {
                    'Fn::Join': [
                      ...
                      [
                        {
                          'Fn::GetAtt': ['teststackbucketprojectsB0C02250', 'Arn'],
                        },
                        '/*',
                      ],
                    ],
    ...
    test('Test CreateAndManageExperiments', () => {
      template.hasResourceProperties('AWS::IAM::ManagedPolicy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Action: ['sagemaker:CreateExperiment', 'sagemaker:DeleteExperiment', 'sagemaker:UpdateExperiment'],
              Effect: 'Allow',
              Resource: 'arn:test-partition:sagemaker:test-region:test-account:experiment/*',
        ...
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              ...
              Effect: 'Allow',
              Resource: `arn:test-partition:ecr:test-region:${CENTRAL_SM_REPO_ACCT}:repository/sagemaker*`,
