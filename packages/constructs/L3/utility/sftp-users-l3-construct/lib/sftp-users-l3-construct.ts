...
const s3IamStatement = function (bucketName: string, homeDirectory: string, partition: string): PolicyStatement {
  ...
  return new PolicyStatement({
    ...
    resources: [bucketArn, `${bucketArn}/${homeDirectory}/*`],
  });
};
...
export class SftpUsersL3Construct extends MdaaL3Construct {
  ...
  constructor(scope: Construct, id: string, props: SftpUsersL3ConstructProps) {
    ...
    for (const userConfig of this.props.users) {
        ...
        [
          {
            ...
            reason:
              'Statement resource is scoped to user home directory. Wildcard is for filenames/object keys not known at deployment time.',
          },
          {
            id: 'NIST.800.53.R5-IAMNoInlinePolicy',
            reason: 'Permissions are specific to this SFTP server role. Inline policy is appropriate.',
          },
          {
