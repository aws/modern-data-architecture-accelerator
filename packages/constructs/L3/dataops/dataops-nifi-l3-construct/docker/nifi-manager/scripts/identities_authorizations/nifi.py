...
def load_existing_identities( existing_policies):
    existing_identities_output = json.loads(
        common.utils.nifi_toolkit("nifi", ["list-users"]))
    existing_identities = {}
...
def load_existing_groups( existing_policies):
    existing_groups_output = json.loads(
        common.utils.nifi_toolkit("nifi", ["list-user-groups"]))
    existing_groups = {}
...
def get_cluster_summary():
    ...
    return json.loads(common.utils.nifi_toolkit("nifi", ["cluster-summary"]))['clusterSummary']


def get_cluster_nodes():
    logger.info(f"Getting cluster nodes")
    return json.loads(common.utils.nifi_toolkit("nifi", ["get-nodes"]))['cluster']['nodes']


def get_root_id():
    logger.info(f"Getting root id")
    return common.utils.nifi_toolkit("nifi", ["get-root-id"])
...
def update(identities_authorizations_config):
    ...
    common.utils.update("nifi", identities_authorizations_config,existing_identities,existing_groups,list(existing_policies.values()))


def cluster_update(identities_authorizations_config):
...
while True:
    ...
    try:
        identities_authorizations_config = replace_root_id(
            common.utils.load_identities_authorizations_config(identities_authorizations_config_filename))
        if not clustered:
            update( identities_authorizations_config)
        else:
            cluster_update(identities_authorizations_config)
