...
export class QuickSightNamespaceL3Construct extends MdaaL3Construct {
    ...
    constructor( scope: Construct, id: string, props: QuickSightNamespaceL3ConstructProps ) {
        ...
        props.federations.forEach( federation => {
            Object.entries( federation.roles ).forEach( roleProps => {
                ...
                this.createNamespaceUserMonitor( this.namespaceName, [ role.roleName ], roleProp.qsUserType, roleProp.qsGroups, roleName, namespaceLambdaUserRole )
            } )
        } )
    }
    ...
    private createNamespaceUserMonitor ( namespace: string, iamRoleName: string[], qsUserType: string, qsGroupNames: string[], roleName: string, namespaceLambdaUserRole: MdaaLambdaRole ): void {
        ...
        eventRule.addTarget( new aws_events_targets.LambdaFunction( namespaceUserFunction, {
    ...
    private createNamespaceUserFunction ( namespace: string, qsUserType: string, qsGroupNames: string[], roleName: string, namespaceLambdaUserRole: MdaaLambdaRole ): Function {
        ...
        const quicksightNamespaceUserLambda: MdaaLambdaFunction = new MdaaLambdaFunction( this, `user-${ roleName }`, {
            ...
            environment: {
                ...
                QUICKSIGHT_GROUPS: qsGroupNames.map( qsGroupName => { return `${ namespace }-${ qsGroupName }` } ).toString()
            },
    ...
    private createNamespace ( name: string, namespaceProvider: Provider ): CustomResource {
        const namespaceResource: CustomResource = new CustomResource( this, `namespace-cr`, {
            ...
            [
                {
                    ...
                    appliesTo: [
                        `Resource::arn:${ this.partition }:ds:${ this.region }:${ this.account }:directory/*`
                    ]
                },
            ],
            true
    ...
    private createReaderManagedPolicy (): ManagedPolicy {
        ...
        NagSuppressions.addResourceSuppressions(
            ...
            [
                {
                    id: 'AwsSolutions-IAM5',
                    reason: 'lakeformation:GetDataAccess does not take resource. QuickSight resource permissions managed in QuickSight.',
                    appliesTo: [
                        `Resource::*`
                    ]
                }
    ...
    private createAuthorManagedPolicy (): ManagedPolicy {
        ...
        NagSuppressions.addResourceSuppressions(
            ...
            [
                ...
                {
                    id: 'AwsSolutions-IAM5',
                    reason: 'redshift:DescribeClusters does not take resource. QuickSight resource permissions managed in QuickSight.',
                    appliesTo: [
                        `Resource::*`
                    ]
                }
