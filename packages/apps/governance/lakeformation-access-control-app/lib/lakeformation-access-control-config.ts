...
export class LakeFormationAccessControlConfigParser extends MdaaAppConfigParser<LakeFormationAccessControlConfigContents> {
    ...
    constructor( stack: Stack, props: MdaaAppConfigParserProps ) {
        ...
        this.grants = Object.fromEntries( Object.entries( this.configContents.grants ).map( configGrantEntry => {
            ...
            return [ configGrantName, {
                ...configGrant,
                databasePermissions: LakeFormationAccessControlL3Construct.DATABASE_PERMISSIONS_MAP[ configGrant.databasePermissions || 'read' ],
                tablePermissions: LakeFormationAccessControlL3Construct.TABLE_PERMISSIONS_MAP[ configGrant.tablePermissions || 'read' ],
    ...
    private resolvePrincipals ( principals: string[] ) {
        const resolvedPrincipals: NamedPrincipalProps = Object.fromEntries( principals.map( configPrincipalName => {
            const configPrincipal = this.configContents.principals[ configPrincipalName ]
            const federationProviderArn = configPrincipal.federationProvider && this.configContents.federationProviders ? this.configContents.federationProviders[ configPrincipal.federationProvider ] : undefined
