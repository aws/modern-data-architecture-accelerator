...
Once a platform architecture is defined, the next step is to translate the architecture into CAEF configuration files.

## CAEF Configuration Structure

CAEF is designed to deploy data environments across multiple domains and environments. Each domain/environment is constituted by one or more configured CAEF modules. Each CAEF module references a CDK app and corresponding configuration. During deployment, CAEF executes a module's underlying CDK application, providing all necessary configuration details as CDK context.
...
* **CDK App** - A *CDK App* is built, executed, and deployed using the AWS CDK framework. The CDK app will be forked from the CAEF orchestrator and executed as a regular CDK application. Each CDK produces one or more CloudFormation stacks, which in turn deploy the cloud resources which will constitute the data environment. Alternatively, instead of deploying resources directly to the envioronment, they can instead be published as Service Catalog products, to be deployed on a self-service basis by users within the accounts.

### Direct Configuration and Deployment

![CAEF Configuration and Deployment](docs/CAEF-Configuration_Deployment.png)

### Configuration and Deployment via Service Catalog

![CAEF Configuration and Deployment via Service Catalog](docs/CAEF-Configuration_Deployment_SC.png)

***

## Sample Architectures and Configurations

These sample CAEF configurations are provided as a starting point for common analytics platform architectures.
...
## CAEF Config File/Folder Layouts

CAEF is configured using a set of YAML configuration files. The main CLI configuration file (typically 'caef.yaml') specifies the global, domain, environment, and modules to be deployed. Module (CDK App) configurations are specified in separate YAML files, or can be configured inline in the CLI config itself. Module (CDK App) configurations are documented in detail in their respective READMEs.

CAEF configuration layouts are very flexible. Configurations for an entire org can be concentrated into a single CAEF config file, or can be spread out across multiple config files by domain, line of business, environment, etc.

### Single Domain, Shared CDK Apps Configs Across Envs

In this scenario, a CAEF config contains a single domain, with CDK App configs shared across dev/test/prod. In this case, the shared configs likely make heavy use of SSM parameters to achieve portability across environments.
...
In this scenario, a CAEF config contains a single domain, with separate CDK App configs across dev/test/prod.
...
### Multiple domains, single CAEF config

In this scenario, multiple domains are in the same CAEF Config/
...
### Multiple domains, Multple CAEF config

In this scenario, each domain is in its own CAEF config.
...
# Additionally, can be referenced in CAEF CDK App configs using the inline {{org}} syntax.
...
# Context values can be referenced in CAEF CDK App configs using the "{{context:<key>}} syntax".
...
# List of custom CDK Aspect implementations which will be applied to all resources produced by all CAEF modules.
...
# (Optional - defaults to latest) The CAEF version can be specified globally,
...
# and will be appended to the CAEF NPM Package name when
# each CAEF CDK App is npm installed.
caef_version: ">=0.15.0"

# One or more domains may be specified. Domain name will be incorporated by default naming implementation
# to prefix all resource names. This allows for multiple CAEF deployments into the same account
...
domains:
  ...
  # The domain name can be referenced within CAEF CDK App configs via the inline {{domain}} syntax.
  shared:
    # (Optional - defaults to latest) The CAEF version can be specified by domain.
    # This should be the NPM package version in standard npm version constraint syntax, 
    # and will be appended to the CAEF NPM Package name when
    # each CAEF CDK App is npm installed.
    caef_version: ">=0.15.0"
    ...
    environments:
      ...
      # It can also be referenced within CAEF CDK App configs via the inline {{env}} syntax.
      dev:
        # (Optional - defaults to latest) The CAEF version can be specified by env.
        # This should be the NPM package version in standard npm version constraint syntax, 
        # and will be appended to the CAEF NPM Package name when
        # each CAEF CDK App is npm installed.
        caef_version: ">=0.15.0"
        ...
        modules:
          ...
          # It can also be referenced within CAEF CDK App configs via the inline {{module_name}} syntax.
          test_glue_catalog:
            # (Optional - defaults to latest) The CAEF version can be specified by module.
            # This should be the NPM package version in standard npm version constraint syntax, 
            # and will be appended to the CAEF NPM Package name when
            # each CAEF CDK App is npm installed.
            caef_version: ">=0.15.0"
          ...
          test_datalake_athena_workgroup:
            ...
            # This approach overrides any caef_version config at the global, domain, env, or module level.
            cdk_app: "@aws-caef/athena-workgroup@>=0.15.0"
            ...
            # App config data can also be directly specified in the caef.yaml
            # Config data specified in the caef.yaml will supercede the contents of the 
            ...
            tag_configs: # Additional tag configs can be specified at the module level
              - module_tags.yaml
            # Tag config data can also be directly specified in the caef.yaml
            # Tag data specified in the caef.yaml will supercede the contents of the 
      ...
      dev:
        ...
        modules:
          test_datalake_roles:
            ...
            cdk_app: "@aws-caef/roles"
            app_configs:
              - ./datalake_domain/roles.yaml

          test_datalake_buckets:
            cdk_app: "@aws-caef/datalake"
            context:
              anycontext: anyvalue
            app_configs:
          ...
          test_datalake_access:
            cdk_app: "@aws-caef/lakeformation-access-control"
            ...
            # otherwise CAEF will lose visibility of the stack in that additional account
  ...
  datascience_domain:
    environments:
      dev:
        ...
        modules:
          service-catalog:
            cdk_app: "@aws-caef/service-catalog"
            app_configs:
              - ./datascience_domain/service-catalog.yaml
          notebook:
            cdk_app: "@aws-caef/sm-notebook"
...
# Optional - Configs for CAEF Devops resources to be deployed when using the '-p' CAEF CLI flag
devops:
  ...
  # The CodeCommit repo containing the CAEF source code
  # Pipelines will be triggered on updates to this repo
  caefCodeCommitRepo: test-caef-repo
  # (Optional) - The branch within the CAEF repo to be deployed
  caefBranch: test-caef-branch
  ...
  pipelines:
    ...
    domain-test1:
      ...
      # -d, -e, and -m params of the CAEF CLI. These filters are effectively ANDed together.
...
Each CAEF Module/CDK App has its own configuration schema, which is documented in their respective READMEs. There are some common configuration behaviours and capabilities, however, which can be used across all CAEF Module configs.

### Dynamic References

CAEF allows use of Dynamic References in configuration files. These build on the concept of CloudFormation Dynamic References.
...
# Will be resolved at synth time to the CDK context value passed from the CAEF CLI config or directly in CDK context
...
# Will be resolved at synth time to the values passed for org/domain/env/account/region from the CAEF CLI config via CDK context
...
CAEF modules may share identical config files across multiple domains, envs, and modules. Because CAEF automatically injects the domain/env/module names into resource naming, each resulting deployment will result in uniquely named resources but with otherwise identical behaviours.

```yaml
# Example CAEF Config With Shared Configs
domains:
  domain1:
    environments:
      dev:
...
```yaml
# Example CAEF CLI Module Specification With Multiple Configs
domains:
  domain1:
    environments:
      dev:
...
CAEF provides a default naming implementation which can be overridden through specification of a custom naming module and class in the CAEF configuration. The default naming convention is implemented as follows:

```text
<organization>-<environment>-<domain>-<module>-<function>
```

* **organization** - Corresponds to the Organization deploying CAEF
* **environment** - Corresponds to the CAEF Environment
* **domain** - Corresponds to the CAEF Domain
* **module** - Corresponds to the name of the CAEF module
