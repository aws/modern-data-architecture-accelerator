...
# Skip lock file sync check when running in npm-ready mode (release pipeline),
# because version_release.sh regenerates the lock file after version bumps
# and the cached node_modules may not match the regenerated lock file.
if [ "$1" != "--npm-ready" ]; then
