...
# Fix: Handle version ranges properly (^, ~, etc.)
find ./packages -type f -name "package.json" | grep -v node_modules | xargs -n1 -I{} sed -i "s/@aws-mdaa\([^\"]*\)\"[~^]*${CURRENT_VERSION}\"/@aws-mdaa\1\"${NEW_VERSION}\"/" {}
sed -i "s/@aws-mdaa\([^\"]*\)\"[~^]*${CURRENT_VERSION}\"/@aws-mdaa\1\"${NEW_VERSION}\"/" ./schemas/package.json
