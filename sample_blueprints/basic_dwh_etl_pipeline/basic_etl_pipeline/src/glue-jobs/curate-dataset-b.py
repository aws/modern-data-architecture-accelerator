...
s3output_dim_table_2 = glueContext.getSink(
  path="s3://"+curated_s3_bucket+"/"+curated_folder+"/dim_table_2",
  connection_type="s3",
  updateBehavior="UPDATE_IN_DATABASE",
  partitionKeys=['ingest_date'],
  compression="snappy",
  enableUpdateCatalog=True,
  transformation_ctx="s3OutputDimQueue",
)
s3output_dim_table_2.setCatalogInfo(
  catalogDatabase=curated_database, catalogTableName="dim_table_2"
)
s3output_dim_table_2.setFormat("glueparquet")
s3output_dim_table_2.writeFrame(dim_table_2_dydf)
